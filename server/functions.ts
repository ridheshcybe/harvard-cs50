import * as types from "./itypes.js";
import { resolve, join, basename } from "path";
import { readdirSync, readFileSync, existsSync } from "fs";

export function fetch(dirtoread: string) {
...
function notes(e: string) {
  ...
  return new Promise<string>((r, j) => {
    const renderthese = resolved.map((e) => {
      ...
      if (e.endsWith(".txt")) return render(data);
...
function getimports(imports: string) {
  return new Promise<types.imports>((r, j) => {
    const data: types.imports = {};
...
function getvidfile(e: string) {
  return new Promise<types.embeds>((r, j) => {
...
export function runfiles(dirtoread: string) {
  const data: types.data = {};
  const [weeks, weeksresolved] = fetch(dirtoread);

  return new Promise<types.data>((r, j) => {
    for (let i = 0; i < weeksresolved.length; i++) {
      ...
      Promise.all(promises).then((e) => {
        ...
        if (typeof notes == "string") assemble.notes = notes;
        if (typeof embeds == "object") assemble.embeds = embeds;
        if (typeof imports == "object") assemble.imports = imports;
 ...
 * @param {Buffer} data
 */
export async function render(data: Buffer): Promise<string> {
